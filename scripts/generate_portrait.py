...
def generate_ascii_portrait():
    ...
    # 3. Curve (Gamma 0.8 to brighten mid-tones)
    ...
    svg_parts = [
        ...
        '<style>',
        '.txt { font-family: ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, Liberation Mono, monospace; font-size: 10px; fill: #c9d1d9; }',
        '.cursor { fill: #58a6ff; }',
        '</style>',
    ...
    for i, line in enumerate(ascii_lines):
        y = i * char_h + 10
        clip_id = f"c{i}"
        duration = 0.06 # Faster typing for high res
        begin_time = i * duration
        
        svg_parts.append(f'<clipPath id="{clip_id}">')
        svg_parts.append(f'<rect x="0" y="-10" width="0" height="16">')
        svg_parts.append(f'<animate attributeName="width" from="0" to="{cols * char_w}" dur="0.3s" begin="{begin_time:.2f}s" fill="freeze"/>')
        ...
        svg_parts.append(f'<animate attributeName="x" from="0" to="{cols * char_w}" dur="0.3s" begin="{begin_time:.2f}s" fill="freeze"/>')
        svg_parts.append(f'<set attributeName="opacity" to="0" begin="{begin_time + 0.3}s"/>')
        svg_parts.append(f'</rect>')
    ...
    with open(output_svg, "w", encoding="utf-8") as f:
        f.write("\n".join(svg_parts))
    print(f"Final Portrait SVG generated at {output_svg}")
